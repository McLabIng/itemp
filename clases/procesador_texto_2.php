...
class Procesador_texto {
    ...
    public static function inserta_registros_test($archivo,$arreglo_alarmas){
        ...
        foreach ($lineas as $linea_num => $contenido)
        {
            ...
            // Fin de busqueda
            $device = snmpget($mi_ip, $mi_comunidad, $mi_oid_temperatura);
            $temp3 = intval(preg_replace('/[^0-9]+/', '', $device));
            $mi_wise = $mi_sitio->getWISE();
            if ($mi_wise <> ''){
                $device2 = snmpget($mi_wise, $mi_comunidad, $mi_oid_temperatura_WISE);
            ...
            // Fin de busqueda
            $device_aux = snmpget($mi_ip_aux, $mi_comunidad, $mi_oid_temperatura_aux);
            $temp3_aux = intval(preg_replace('/[^0-9]+/', '', $device_aux));
            $mi_wise_aux = $mi_sitio_aux->getWISE();
            if ($mi_wise_aux <> ''){
                $device2_aux = snmpget($mi_wise_aux, $mi_comunidad, $mi_oid_temperatura_WISE);
